...
// - `cargo add --features backtrace anyhow`
//
// References:
...
pub fn main() -> Result<()> {
    ...
    for epoch in 1..(1 + EPOCHS) {
        println!("+ Processing Epoch {} (BATCH_SIZE: {}):", epoch, BATCH_SIZE);
        let mut sum_loss = 0.0;
        let mut cnt_loss = 0.0;
        let bar = ProgressBar::new((data.data().numel() / BITS_PER_BYTE / BITS_PER_CHAR).try_into().unwrap());
        bar.set_style(
        ...
        for batch in data.iter_shuffle(SEQ_LEN + 1, BATCH_SIZE) {
            bar.inc(1);
            // println!(" >> Processing... | sum_loss: {}, cnt_loss: {}", sum_loss, cnt_loss);
